...
def tt():
     ...
     while(x < 11):
          if(flag == 0):
               for w  in g_l:
                    for m in b_l:
                         log_maker(w.name_girl + ' is searching '+ m.name_boy)
                         if w.r_s == 'single' and  m.r_s == 'single' and (w.chk_elg(m.budget_for_girlfriend) and (m.chk_elg(w.maintence_budget_girl,w.attraction_girl))):
                              w.r_s = 'in_relationship'
                              m.r_s = 'in-relationship'
                              w.bf = m.name_boy
                              m.gf = w.name_girl
                              log_maker(w.name_girl+' is in relationship with ' + m.name_boy)
                              c_l = c_l + [(m,w)]
                              flag = 1
                              break
                    if flag == 1:
          ...
          if(flag == 1):
               for a in b_l:
                    ...
                    if(a.r_s == 'single'):
                         for  b in g_l:
                              if(f < b.attraction_girl and  b.r_s == 'single'):
                                   p = b
                                   f = b.attraction_girl
                    break
               flag = 0
               ...
               p.bf = a.name_boy
               a.gf = p.name_girl
               logging.info('girl '+ b.name_girl+' in realationship '+a.name_boy)
          x = x+1
     for m in g_l:
          if m.r_s == 'single':
